...
class RPC(EncoderDecoderMixin):
    ...
    async def connect(self):
        ...
        主要执行的操作有:
        + 将监听响应的协程response_handler放入事件循环
        + 如果有验证信息则发送验证信息
        + 获取连接建立的返回
        ...
        self.reader, self.writer = await asyncio.open_connection(
            ...
            loop=self.loop)
        self.closed = False
        self._response_task = asyncio.ensure_future(self.response_handler())
        query = {
            "MPRPC": self.VERSION,
            "AUTH": {
    ...
    def status_code_check(self, response: Dict[str, Any]):
        """检查响应码并进行对不同的响应进行处理.
        ...
        if code >= 500:
            if self.debug:
                print("server error")
            self.server_error_handler(code)

        elif 500 > code >= 400:
            if self.debug:
                print("call method error")
            self.method_error_handler(response)

        elif 400 > code >= 200:
            if code >= 300:
                self.warning_handler(code)
            if code in (200, 201, 202, 206, 300, 301):
                if self.debug is True:
                    print("resv resp {}".format(response))
                self.method_response_handler(response)

        elif 200 > code >= 100:
            self.server_response_handler(response)
        else:
            raise MprpcException("unknow status code {}".format(code))
        return True

    def server_error_handler(self, code: int):
    ...
    def method_error_handler(self, response: Dict[str, Any]):
    ...
    def warning_handler(self, code: int):
    ...
    def method_response_handler(self, response: Dict[str, Any]):
        ...
        code = response.get("CODE")
        if code in (200, 300):
            self.result_handler(response)
        else:
            asyncio.ensure_future(self.gen_result_handler(response))
            #self.gen_result_handler(response)

    def server_response_handler(self, response: Dict[str, Any]):
    ...
    # ---------------------应答结果响应处理-------------------
    def result_handler(self, response: Dict[str, Any]):
        """应答结果响应处理.
    ...
    async def gen_result_handler(self, response: Dict[str, Any]):
        """流式结果响应处理.
    ...
    async def wrap_gen(self, ID: str):
        """异步迭代器包装.
